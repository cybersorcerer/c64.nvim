...
function M.debug(config)
  ...
  if x64_running then
    ...
    if files.sym_exists then
      vim.notify(
        string.format("Starting VICE minimized with remote monitor and symbols: %s", vim.fn.fnamemodify(files.sym, ":t")),
        vim.log.levels.INFO
      )
    else
  ...
  vim.defer_fn(function()
    ...
    local wrapper_cmd = string.format([[
      ...
      printf '\033[1;93m║  VICE Monitor - Close with: <Esc><Esc>q                          ║\033[0m\n'
      printf '\033[1;93m╚══════════════════════════════════════════════════════════════════╝\033[0m\n'
      printf '\n'
    ...
    if files.sym_exists then
      vim.defer_fn(function()
        ...
        vim.notify(
          string.format("Loaded %d symbols. Program loaded but not started. Use 'g 0801' to run.", #sym_content),
          vim.log.levels.INFO
        )
      end, 500)
    end
